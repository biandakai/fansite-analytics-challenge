...
def main():
	input_path, hosts_path, hours_path, resources_path, blocked_path = sys.argv[1:] # Get input and output file path
	cwd = os.getcwd() # Get current directory

	df = read_log_txt(file_path=os.path.join(cwd, input_path.replace('./', ''))) # Read log text, return dataframe
